        ...
        const templates = {
            home: () => `
                <div id="home-view" class="view">
                    <div class="hero-section" style="background-image: url('https://files.catbox.moe/03g5k9.gif')">
                    ...
                    <form class="search-form card" data-action="search">
                        <input type="text" id="search-input" name="query" placeholder="Cari judul anime..." required>
            ...
            renderComments: (snapshot, animeUrl, episodeUrl) => {
                ...
                container.innerHTML = comments.reverse().map(c => `
                    <div class="comment">
                        ...
                        ${state.currentUser && state.currentUser.uid === c.uid ? `<button class="delete-comment-btn" data-action="delete-comment" data-key="${c.key}" data-anime-url="${animeUrl}" data-episode-url="${episodeUrl}"><i class="fas fa-trash"></i></button>` : ''}
                    </div>`).join('');
            },
...
// === BAGIAN 3 DARI 3 DIMULAI ===
    ...
    async function switchView(viewName, params = {}) {
        ...
        try {
            ...
            mainContent.innerHTML = UI.render('subscribe'); // Fallback
        } finally {
            showLoading(false);
        }
    }
